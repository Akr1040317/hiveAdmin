  ...
  ({ className, variant = 'primary', size = 'md', accent = false, ...props }, ref) => {
    ...
    const getAccentStyles = () => {
      ...
      if (accent === true) {
        ...
        return 'bg-violet-600 text-white hover:bg-violet-700 focus:ring-violet-500';
      }
      const accentMap = {
    ...
    const variants = {
      primary: accent ? getAccentStyles() : 'bg-gray-700 text-white hover:bg-gray-600 focus:ring-gray-500',
      secondary: 'bg-background-card border border-border text-gray-300 hover:bg-background-hover focus:ring-gray-500',
      ghost: 'text-gray-400 hover:text-gray-200 hover:bg-background-hover focus:ring-gray-500',
    };
